...
### Fixed
- Fixed Confluence content truncation when multiple code blocks have mixed language parameters
  - Root cause: optional language group in regex could span across multiple code blocks
  - When code block without language was followed by one with language, all content between was lost
  - Refactored regex to capture parameters separately and extract language in replacement block
  - This constrains language extraction to only the current code block's parameter area
