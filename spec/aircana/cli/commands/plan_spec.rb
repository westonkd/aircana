...
require "spec_helper"
require "aircana/cli/commands/plan"

RSpec.describe Aircana::CLI::Plan do
  let(:claude_agents_dir) { File.join(Aircana.configuration.claude_code_config_path, "agents") }
  let(:planner_agent_path) { File.join(claude_agents_dir, "planner.md") }
  ...
  describe ".run" do
    context "when planner agent exists" do
      let(:claude_path) { File.expand_path("~/.claude/local/claude") }

      before do
        allow(File).to receive(:exist?).with(planner_agent_path).and_return(true)
        allow(File).to receive(:executable?).and_return(false)
        allow(File).to receive(:executable?).with(claude_path).and_return(true)
        allow(described_class).to receive(:system).and_return(true)
      end

      it "launches claude with planner prompt" do
        planner_prompt = "#{claude_path} \"Start a planning session with the 'planner' sub-agent\""
        expect(described_class).to receive(:system).with(planner_prompt)

        described_class.run
      end
    end

    context "when planner agent does not exist" do
      let(:claude_path) { File.expand_path("~/.claude/local/claude") }

      before do
        allow(File).to receive(:exist?).with(planner_agent_path).and_return(false)
        allow(File).to receive(:executable?).and_return(false)
        allow(File).to receive(:executable?).with(claude_path).and_return(true)
    ...
    context "when claude command is not available" do
      ...
      it "shows error message with manual instructions" do
        expect(Aircana.human_logger).to receive(:info).with("Launching Claude Code with planner agent...")
        ...
        info_msg = "You can manually start Claude Code and run: " \
                   "Start a planning session with the 'planner' sub-agent"
